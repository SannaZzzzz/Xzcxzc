  ...
  try {
    ...
    try {
      ...
      if (!completion || typeof completion !== 'object') {
        console.error('阿里云百炼API返回了非预期的响应格式');
        throw new Error('API返回了非预期的响应格式');
      }
      ...
      // 检查是否收到了HTML响应（通常表示重定向或认证问题）
      if (apiError.message && (
        apiError.message.includes('<!DOCTYPE html>') || 
        apiError.message.includes('<html>') ||
        apiError.message.includes('text/html')
      )) {
        console.error('收到HTML响应而非JSON，可能是API认证问题');
        return res.status(200).json({
          choices: [
            {
              message: {
                content: "抱歉，服务器通信出现问题。这可能是由于API密钥配置或认证问题导致的。作为桥吊专家，我建议尝试刷新页面或稍后再试，或直接联系技术支持人员确认API配置。"
              }
            }
          ]
        });
